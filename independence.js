/* ==========================================================================
   ...
   6. Zeel Rupapara Personal Wish Card Generator & PNG / PDF Downloader
      (Dark/White Themes, Landscape Export, In-App Browser Photo Save Modal & Web Share API)
...
/* --------------------------------------------------------------------------
   6. ZEEL RUPAPARA WISH CARD GENERATOR & LANDSCAPE MOBILE EXPORT (PNG/PDF)
      INCLUDES SMART PHOTO PREVIEW MODAL & WEB SHARE API FOR IN-APP BROWSERS
   -------------------------------------------------------------------------- */
...
function initZeelWishes() {
  ...
  const modalDirectDownload = document.getElementById("modalDirectDownloadBtn");
  ...
  // Web Share API handler for Modal
  if (modalShareBtn) {
      ...
      if (currentBlobFile && navigator.canShare && navigator.canShare({ files: [currentBlobFile] })) {
        try {
          await navigator.share({
            ...
            text: "My Independence Day Greeting Card — Wishes from Zeel Rupapara",
        ...
        try {
          await navigator.share({
            title: "Happy 80th Independence Day!",
            text: `Happy Independence Day! Wishes from Zeel Rupapara`,
        ...
        alert("📸 Long press the image to save it directly to your Photos / Gallery!");
      }
    });
  }

  // Download Card as Photo (PNG) — Supports Mobile In-App Browsers (Instagram/Safari/Chrome)
  if (downloadImgBtn) {
    downloadImgBtn.addEventListener("click", () => {
      const visitorName = nameInput.value.trim() || "ProudIndian";
      const fileName = `Azaadi_WishCard_${visitorName.replace(/\s+/g, '_')}.png`;
        ...
        renderLandscapeWishCardCanvas(wishCardEl).then(canvas => {
          ...
          canvas.toBlob((blob) => {
            if (blob) {
              ...
              // Set preview image & download link in modal
              if (modalImg) modalImg.src = currentDataUrl;
              if (modalDirectDownload) {
                modalDirectDownload.href = blobUrl;
                modalDirectDownload.download = fileName;
              }

              // Show Photo Preview Modal for Mobile / In-App Browsers
              if (photoModal) {
                photoModal.classList.remove("hidden");
              }

              // Attempt Web Share API automatically if on mobile device
              if (navigator.canShare && navigator.canShare({ files: [currentBlobFile] })) {
                navigator.share({
                  title: "Happy 80th Independence Day!",
                  text: "Independence Day Greeting Card — Wishes from Zeel Rupapara",
                  files: [currentBlobFile]
                }).catch(() => {});
              }
          ...
          downloadImgBtn.innerHTML = `<i class="fa-solid fa-check"></i> Photo Ready!`;
          setTimeout(() => {
  ...
  // Download Card as PDF — High-Res Landscape Format
  if (downloadPdfBtn) {
    downloadPdfBtn.addEventListener("click", () => {
      const visitorName = nameInput.value.trim() || "ProudIndian";
      const fileName = `Azaadi_WishCard_${visitorName.replace(/\s+/g, '_')}.pdf`;
